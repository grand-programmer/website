                    ...
                    <v-card>
                        ...
                        >
                            <template v-slot:item.filename="{item}">
                                <span :href="'/public/docfiles/' + item.filename" target="_blank">{{ '/public/docfiles/' + item.filename}}</span>
                            ...
                            <template v-slot:item.actions="{ item }">
                                <v-btn :to="'/admin/files/edit/' + item.id" color="primary" small>
                                    <v-icon
                                        small
                                        class="mr-2"
...
export default {
    ...
    data: () => ({
        ...
        headers: [
            ...
            {text: 'Файл номи', sortable: true, value: 'name', align: 'start', width: '30%'},
            {text: 'Урл адреси', sortable: true, value: 'uuid', align: 'center'},
            {text: 'Файл номи', sortable: true, value: 'filename', align: 'center'},
    ...
    methods: {
        initialize() {
            api.readFiles(this.options).then((response) => {
        ...
        deleteItemConfirm() {
            api.deleteFile(this.editedItem.id).then(response => {
                if (response.status === 200) {
                    this.initialize();
